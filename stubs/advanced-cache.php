...
// Paths are derived from WP's own constants instead of being hardcoded by WP
// Rocket. Deployer swaps `current` to a new releases/N directory on every
// deploy, and __DIR__ resolves through that symlink, so any absolute path baked
// in here points at a release that deploy:cleanup will eventually delete. Once
// that happens the file_exists() guard below trips and page caching silently
// turns off. Keep this file dynamic and tracked in git so every release has it.
//
// Only WP_CONTENT_DIR is usable: wp-settings.php includes this drop-in at line
// ~100, long before wp_plugin_directory_constants() defines WP_PLUGIN_DIR at
// line ~488, so referencing that constant here is a fatal error.
...
$rocket_mobile_detect = $rocket_path . 'inc/classes/dependencies/mobiledetect/mobiledetectlib/Mobile_Detect.php';
if ( file_exists( $rocket_mobile_detect ) && ! class_exists( 'WP_Rocket_Mobile_Detect' ) ) {
	include_once $rocket_mobile_detect;
}
